
<header>
	<!--<div>
		<h3 class="color_blanco">Portafolio de</h3>

	<h2 class="titulo_principal color_secundario en_linea align_left">Proyectos</h2>
	<h3 class="en_linea color_blanco">Informáticos</h3>
	</div>-->
	
	<div class="ms-hide">
		<i class="fa fa-coffee color_blanco" style="font-size:30px;float:left;margin-left:20px;margin-top:10px;" aria-hidden="true"></i>
	</div>
	
	<section id="navegacion">
		<a href="#contacto"><h3>Contacto</h3></a>
		
		<a href="#proyectos"><h3>Proyectos</h3></a>
	</section>
	<div class="clear"></div>
</header>